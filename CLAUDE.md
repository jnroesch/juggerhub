...
`specs/033-umami-analytics/plan.md` (Self-hosted Umami analytics — privacy-friendly, cookieless usage measurement, run in-cluster beside the app. **No backend changes at all**; the change surface is Terraform, nginx config, docker-compose, and one injected `<script>`. Exposure is **split**: the tracker script + collection endpoint are proxied **same-origin** through the existing frontend nginx at **renamed paths** (`/jh-insights.js`, `/jh-insights/e`) via Umami's *runtime* `TRACKER_SCRIPT_NAME`/`COLLECT_API_ENDPOINT`, which is what defeats blocklists; the **dashboard gets its own hostname** (`analytics[-dev].juggerhub.com`) because Umami's `BASE_PATH` is **build-time** and a same-origin dashboard would mean forking/rebuilding a third-party Next.js app — and `/api/` on this origin is already the .NET backend. Data lives in a **new `umami` database on the existing Postgres StatefulSet** under a scoped non-superuser role that is explicitly `REVOKE`d from `appdb` (Postgres grants `CONNECT` to `PUBLIC` by default, so the revoke is load-bearing). Provisioned by an **initContainer** — `/docker-entrypoint-initdb.d/` only runs on an empty data dir and would be a **silent no-op** on the already-initialised Dev/Prod volumes. Per-env config reaches the already-built frontend image at container start via the nginx image's **own** `envsubst` template mechanism + `sub_filter` on `</head>` — adds **no `.sh` file** (constitution VI) and no Angular source change; all placeholders are `JH_`-prefixed so envsubst cannot clobber `$host`/`$uri`/etc. FR-007 needs a **GPC guard** in the injected snippet — Umami's `data-do-not-track` covers DNT only. Umami has **no env var for the admin password**, so a post-deploy Job writes a bcrypt hash from GitHub Environments (FR-022). **Top risk, gates everything: Umami v3 ships Prisma 6.18 but PG18 support is only confirmed upstream for Prisma 7.2** — verify locally against `postgres:18.3-alpine` before any Terraform. **Session replay must be OFF** (v3 feature; would capture member data on an auth-only platform). Owner decision FR-008: page paths recorded **verbatim** incl. `/players/<handle>`, so the store holds subject-side personal data; privacy policy deferred → GH #92.).
<!-- SPECKIT END -->
