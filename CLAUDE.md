...
`specs/032-city-search-ranking/plan.md` (City search relevance ranking — re-rank the city-picker options from `GET /api/v1/cities/search` so the city people usually mean comes first. Within the existing match-quality tier (exact name/ascii-prefix hits above alternate-name/exonym hits — **preserved**), order by **distance from the signed-in user's stored home city → population DESC → existing name-length/name tiebreakers**. Proximity origin is resolved **server-side** from `PlayerProfile.HomeCity` — **no** browser geolocation, **no** permission prompt, **no** new request param; users without a home city skip the distance tier and fall back to population. Ranking runs in SQL across the **full candidate set before the `MaxResults` cap** via an **equirectangular squared-distance** term `(Δlat)² + (cos lat₀·Δlon)²` — pure arithmetic EF translates, no PostGIS/trig/C#-haversine. Adds `CityReference.Population` (`int`, default 0) from GeoNames cities500 col 14 → regenerate the bundled seed + one-time reseed. `SearchAsync` gains a nullable `userId`. Backend-only; option DTO/label shape unchanged.).
<!-- SPECKIT END -->
