...
`specs/034-image-processing-pipeline/plan.md` (Server-side image processing pipeline — GH #98; first ticket of the image-handling split from #13, siblings #97 storage / #99 galleries. Introduces a reusable, owner-agnostic **`IImageProcessor`** service in a **new `backend/Services/Media/`** namespace that every uploaded image passes through before storage: content-type allow-list (PNG/JPEG/WebP) → **header-only pixel guard via `Image.Identify` BEFORE decode** (decompression-bomb protection, `MaxDecodePixels` ~40MP) → `AutoOrient()` + strip **all** EXIF/GPS/ICC/XMP/IPTC metadata → **per-context resize** (avatar=**square-crop** center, future gallery=**fit**; never upscale) → **re-encode to WebP** (quality 80) → reject if over the stored-output ceiling. Library: **SixLabors.ImageSharp** (pure-managed → trivial env parity, no native binary), behind the `IImageProcessor` seam so it's swappable; **Six Labors Split-License** is an owner decision, SkiaSharp is the BSD fallback. This deliberately **supersedes the 003-profile "no new NuGet packages" bar** (recorded drift). **No DB/schema change and no migration** — `ProfileAvatar` still stores `bytea`+`ContentType`; only the *bytes written* change (normalized WebP). `ProfileService.SetAvatarAsync` is rewired to call the processor; `AvatarSetStatus` gains **`DimensionsTooLarge`** + **`Unreadable`** for distinct rejection reasons (controller mapping unchanged — non-success→400+reason). **Input cap stays generous (~8MB `[RequestSizeLimit]`) vs a small stored-output ceiling (~512KB)** — accept a big phone photo, store small. Processing is **synchronous/in-request** (uploads are rare, one-per-member) — no queue/worker. Animated inputs accepted and **flattened to first frame**. **Known test drift**: `ProfileTests.Avatar_upload_accepts_a_valid_png` must change its served-type assertion `image/png`→`image/webp`. Caching headers on GET avatar are out of scope (→ #97).).
<!-- SPECKIT END -->
