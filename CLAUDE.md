...
`specs/054-pwa-shell/plan.md` (PWA shell for web push — GH #307, the prerequisite for #308 (push channel) and #309 (chat push). The app is a plain SPA — no manifest, no service worker, no registration — so it cannot be installed and the browser has nowhere to deliver push; on iOS push reaches ONLY a Home-Screen-installed app, which is why installability is not decoration. **FRONTEND + WEB TIER ONLY: no backend, no entity, no endpoint, no migration, no new dependency, no infra change** — if a task touches `backend/` or `infra/`, something is wrong. **THREE OWNER DECISIONS (spec Clarifications)**: (1) a **hand-written PUSH-ONLY `public/sw.js`, NOT `@angular/service-worker`** — two lifecycle listeners (`install`→`skipWaiting`, `activate`→`clients.claim`), **no `fetch` handler, no `caches`, no `importScripts`**, enforced by a guard spec that reads the file; the second reason beyond 026's "nothing to show offline" is that 033's `sub_filter` injects per-environment analytics into `index.html` at serve time and a caching worker would freeze it on the device; (2) **NO install prompt/banner/copy, zero interface strings** — the manifest deliberately has **no `description`/`lang`/`screenshots`/`shortcuts`** (Chrome would render them in the install dialog); the "add to Home Screen" affordance ships with #308's enable-push flow; (3) **NO offline mode, ever** (FR-011), made structural by the same guard. **THREE FACTS FOUND BY READING**: (a) the nginx image's `mime.types` has **no `webmanifest` entry** (checked inside `nginxinc/nginx-unprivileged:1.31.5-alpine`) → the manifest location sets `default_type application/manifest+json`; (b) the privacy policy's "what's kept in your browser" list is **exhaustive** ("that's all there is here") and a registered worker is visible in the browser's site data → **FR-015 added during planning**: one generic durable paragraph in `storage.body` + the `legalBasis` enumeration, ×3 locales, German authoritative, `legal-catalog.spec.ts` walks arrays so all three at once; (c) the existing `env(safe-area-inset-bottom)` paddings are **0** because the viewport meta has no `viewport-fit=cover` — **leave it that way**: default viewport boxes the standalone app inside the safe areas (= FR-004); `cover` would put the sticky top bar under the status bar with no top-inset handling anywhere. Files: `public/manifest.webmanifest` (`id`/`start_url`/`scope` "/", `display` standalone, `background_color #FBF8F3` = DESIGN `surface-page`, `theme_color #FFFFFF` = `surface-card` — the bars are white, coral was rejected), `public/icons/` (192/512 `any` rendered from `favicon.svg` with transparent corners; 192/512 `maskable` + **180 opaque `apple-touch-icon.png`** from a NEW full-bleed `icon-maskable.svg` — iOS takes the icon from `<link rel=apple-touch-icon>`, not the manifest, and paints transparency BLACK; `any`/`maskable` on SEPARATE icons per Chrome guidance), `core/pwa/register-service-worker.ts` (`register('/sw.js', {scope:'/', updateViaCache:'none'})` after `load`, ONE attempt, `.catch(()=>{})`, called from `main.ts` `.then()` after bootstrap — not an app initializer, not a component), `index.html` +3 lines (manifest link, theme-color meta, apple-touch-icon; viewport UNCHANGED; no `mobile-web-app-capable` meta unless the iOS walk shows Safari chrome — the one recorded contingency), `nginx.conf.template` +2 **exact-match** locations beside `/i18n/` (`try_files $uri =404` + `Cache-Control no-cache` — the 038 shadowing hazard: a shadowed `sw.js` is `index.html` served as JS and registration fails SILENTLY because FR-008 swallows errors), `tools/render-pwa-icons.mjs` (one-off via `playwright-core`, already installed with its Chromium — the CLI has no `--omit-background`; `.mjs` not `.ps1` is a recorded VI deviation with the `regenerate-cities500.mjs` precedent; PNGs committed, guard spec checks PNG IHDR sizes). Tests: unit spec for the helper (fake Navigator/Window), guard spec (`pwa-shell.spec.ts` reads sw.js/manifest/index.html/PNGs from disk), e2e `web-e2e/src/pwa-shell.spec.ts` at both projects (`/sw.js` is JS not HTML; manifest + icons 200; `navigator.serviceWorker.ready` scope = origin+'/'; `cache-control` asserted ONLY when `BASE_URL` is set — the Vite dev server sends its own headers). **Chrome no longer needs a SW for installability** (108+), so US1/US2 are independent. **⚠ Principle VII NOT engaged** — the worker fetch is the browser's own same-origin static request, one attempt by FR-008; any retry/timeout wrapper is review-rejectable. **Gate 7 NOT instantiated** (045 precedent: no markup, no interface copy) — the verification is the owner's **on-device walk**: Android install + iPhone Add-to-Home-Screen, standalone, `/chat` and `/admin` bottom bar clear of the home indicator, sign-in required inside the installed app (separate cookie jar — platform fact, never "fix" it), screenshots to the PR. Residuals: Chromium-only e2e; icon bytes stable only per Chromium version (guard checks size, not pixels); SC-008 reworded — browsers also fetch the icons.)

Prior plan, MERGED (2026-09-19, PR #326): `specs/053-invite-aware-onboarding/plan.md` (Invite-aware onboarding — GH #324. A person who opens a team's **shared invite link** with no account registers, verifies, signs in and walks the wizard, and the product has forgotten they were invited. **THE ISSUE'S DIAGNOSIS WAS HALF WRONG, CORRECTED BY READING THE CODE**: the `returnUrl` chain from sign-in onward ALREADY works (`sign-in.component.ts:58-63` carries it into `/onboarding`, `onboarding.component.ts:381` honours it on exit) — the trail dies at exactly two links: `RegisterComponent` only decorates its own sign-in links and never sends the returnUrl to the server, and `VerifyEmailComponent`'s success button is a bare `/sign-in`. Two owner decisions (spec Clarifications): **carry the invitation THROUGH THE SERVER, NEVER STORE IT** (no entity, no column, no migration, no endpoint — `RegisterRequest` + `ResendVerificationRequest` gain optional `InviteSlug`/`InviteToken`, `AuthEmailService.BuildLink` appends `&inviteSlug=&inviteToken=`; account-storage and browser-storage both declined), and **accepting inside the wizard STAYS in the wizard**, the exit lands on `/t/{slug}`. **THE CARRIED THING IS AN IDENTITY, NEVER A URL** — the two path segments of `/join/{slug}/{token}`, validated for SHAPE ONLY by a pure `InviteReference.TryParse` (slug via the existing `TeamSlugPolicy`, token `^[A-Za-z0-9_-]{16,128}$` — `NewToken()` yields 43 base64url chars; bounds, not `{43}`); registration NEVER reads the invitation (no second oracle beside the anonymous preview), a malformed pair is dropped silently and the neutral `NeutralCheckEmail` response is unchanged in every branch. Thread the reference through **all three** verification-send paths (new account, the existing-unverified-email resend branch at `AuthService.cs:101-114`, and `ResendVerificationAsync`) — the invited person registering twice from the same link is the realistic case for branch two. Frontend: ONE util `core/utils/invite-ref.ts` (parse from a `/join` returnUrl, parse from query, compose `?action=accept` and the bare page path) shared by register / sign-in / verify-email / onboarding, mirroring `safeReturnUrl`'s reason to exist. Wizard: all new state is **signals** (zoneless, 045's lesson); preview via existing `GET /invitations/{token}`, addressed invites via existing `listMine()` (token included), accept via existing `POST …/accept`; **029's Continue-is-pure-navigation rule is preserved** — Accept is its own press, no state disables Continue/Skip/Back. **`MembershipService` is NOT loaded on `/onboarding`** (off-shell) — load it only when an invitation is known, because accept answers 200 for both Joined and AlreadyMember and only the list can tell them apart (FR-021); no invitation ⇒ ZERO extra requests, the plain step stays byte-identical. **EXIT RULE (R5)**: joined → `/t/{slug}`; carried+usable+walked past Welcome → `/join/{slug}/{token}` WITHOUT `action=accept` (the wizard offered it and the player chose not to press — `InviteAcceptComponent.maybeResume` would otherwise auto-accept, making Continue an accept by another route); dismissed at Welcome → today's behaviour incl. auto-accept; carried but stale → `/`. Dedupe addressed invites by `teamSlug` against the carried one. **Invite-row markup is COPIED from `my-team.component.html:48-56`, not extracted** (029 precedent — presentation, no rule inside; 052 extracted invite-search because its `@switch` encoded a rule) — "My team" and the invite page are NOT touched. Accept is `variant="secondary"` (one coral CTA per view = Continue). ~12 keys `onboarding.team.invite.*` × 3 catalogues at once (`catalog-parity.spec.ts`), German `–` not `—`; the joined confirmation must NOT reuse 029's "an admin still has to say yes". **⚠ Principle VII NOT engaged** (no outbound call; browser accept/decline are POSTs never auto-retried — FR-022's retry is a press). **Gate 7 IS engaged** → `checklists/ui-review.md`; binding case is the invitation card at 375px in German. Residuals recorded: cross-device limit (verify on one device, cold sign-in on another ⇒ plain step — owner-accepted); `/join/{slug}/{token}` is a PATH so 033 analytics records the token today (pre-existing, follow-up issue); a targeted invite hidden behind an accepted link invite stays Pending until it expires.)

Prior plan, MERGED (2026-09-19): `specs/052-team-creation-wizard/plan.md` (Team creation wizard — GH #320. Creating a team is the last "create" flow still on a single dense screen; it becomes a **five-step wizard** in the house style (name+handle → type+city → **review** → logo → invite), and two capabilities that already exist get placed inside it. **FRONTEND ONLY: no entity, no column, no migration, no endpoint, no DTO field, no new dependency** — if a task produces an `Add-Migration` or touches `backend/`, something is wrong. All four calls are ones other screens already make (`slug-available`, `POST /teams`, 051's `PUT /teams/{slug}/logo`, the invite `user-search` + `POST .../invitations`). **THE REQUEST'S THIRD ITEM WAS REJECTED BY READING THE CODE**: "move the description to that screen" cannot be done — **a team has no description anywhere** (not on `Team`, not in `backend/Dtos/Teams/`, not in any migration, not in team settings, not on `TeamDetail`), so it would be an additive field with its own migration and display surfaces, **OUT of scope by owner decision → GH #321**. **THE LOAD-BEARING DECISION is WHERE THE TEAM IS CREATED**: both optional steps are **slug-addressed and admin-gated server-side**, so neither can run before the team exists — the team is therefore created **at the review step**, and `createdSlug` is a **latch** (set once, never cleared) that the whole post-create half derives from: Back is **not rendered** once set (the handle is `init`-only — immutable), the optional steps cannot be entered without it, and both finish by navigating to `/t/{slug}`. Deriving it from *the team existing* rather than from `stepIndex >= 3` is what keeps it one fact instead of three conventions. **THE 409 IS THE ONLY SAFE DISCRIMINATOR** for "address taken between check and create" — `TeamsController.Create` maps `CreateTeamStatus.SlugTaken` to **409** and everything else to **400**, so branch on `err.status === 409` (→ back to the address step, other answers kept) and **never on the message**: the server's `detail` is English-only, which is the documented reason the availability check ships a machine-readable `reason` code the client turns into `teams.create.slugReason.*`. **051 APPLIES A LOGO IMMEDIATELY AND SO DOES THIS** — `team-settings.onLogoSelected()` uploads straight from the `change` handler and renders the result through `TeamService.logoUrl`'s **per-slug revision counter** (the GH #283 fix: a logo's URL doesn't change when its image does, so without bumping it the new logo never appears); onboarding's hold-the-`File`-and-upload-at-finish model is right only where nothing exists to upload *to*, so **FR-016 was amended in the spec** rather than left as drift. The invite search is **extracted, not copied**, to `features/teams/components/invite-search/` (two call sites; the `Member`/`Invited`/`Invitable` `@switch` is a rule about whether an invite may be offered, and 046 recorded that duplication class drifting once already) — the extraction also **adds the empty state team-invitations never had** (FR-025), a deliberate change to an existing screen, called out so it isn't read as a regression. The slug pipeline (`distinctUntilChanged` → `tap(clear)` → `debounceTime(300)` → `switchMap` with `catchError` **inside**) moves **unchanged, comments included** — it is three bug fixes encoded as operator order. App is **zoneless**, so anything the template reads for an enable/disable decision must be a signal (045's lesson). **⚠ Principle VII is NOT engaged** — no outbound call, no new kind of browser→backend call; wrapping these in `AddJuggerHubResilience` is review-rejectable. What it *does* say applies is a **prohibition**: create/upload/invite are all mutations, **never auto-retried on the browser hop** (a replayed `POST /teams` makes a second team) — FR-010/FR-018's retry is a *press*. **Gate 7 IS engaged** (almost all new markup + new copy ×3 catalogues) → `checklists/ui-review.md`; the binding case is the **new review step in German at 375px**. Recorded deviations: **no draft persistence** unlike the other two wizards (045) — two short pre-create steps, not 21 answers, follow-up; and an unrelated screen is touched for the extraction.)

Prior plan, not yet implemented: `specs/050-tournament-results/plan.md` (Tournament results — GH #295. Tournament events get a **results** section — a final ranking (ties as standard competition ranking 1,2,3,3,5, stored normalised) and, for Tugeny imports, every match with set scores — and teams get a **placement history**. Works WITH **Tugeny** (the tournament-day bracket tool): team list goes IN as plain text one-per-row (its *Import Team Names* accepts it), results come OUT by **pasting** its *Export Ranking for JTR* JSON or **importing** from its public MIT API once finalized. **THE REPO IS PUBLIC — never write any JuggerHub-vs-JTR positioning into specs, issues, commits or code.** **3 new tables** (`TournamentResult` 1:1 Event with the Tugeny link + provenance, `TournamentPlacement`, `TournamentMatch`), **1 migration**, **no new package**. **OWNER DECISIONS (spec Clarifications)**: past tournaments IN scope (past-dated events); a placement stays a **plain name** until connected, and only two actors may connect — an **event admin, only to a team with a `Joined` `EventSignup` at that event**, or a **platform admin, to any team** (this covers teams that PLAYED but have no JuggerHub sign-up: every team of a past tournament, sign-ups run elsewhere, guests; say "without a JuggerHub sign-up", never "didn't enter"); **teams can NEVER claim**; and **NO bulk/remembered mapping, not even by Tugeny team id — one placement at a time, no suggestions** (owner: "rather check manually than backfill wrong data"). **Past-dated events already work server-side** (no past-date rule anywhere; every signup/party/market path closes at `EndsAt < now`) — the only fix is the FRONTEND still showing Join/Enter-party on ended events (`join-actions`, `PartyService` `CanForm` ignores `IsEventOpen`). **Formats were read from Tugeny's binary, not guessed**: `composeJson` references exactly `position`+`name` (export = `[{name,position}]`, refuses unless every rank is decided); never learn anything from turniere.jugger.org (its terms forbid automated reading). **Tugeny API quirks (measured over all 46 finalized)**: unknown slug = **HTTP 200 with body `null`**; unfinalized = `rankings: []`; `rankings` is an object `"1".."N"` (no ties ever); 318/3510 matches are **draws** (`victorious_team_id: null`); knockout matches have `group: null` (round only in `name`); matches are NOT time-ordered; largest body 246 KB. Samples in `contracts/tugeny-samples/`. **⚠ Principle VII IS ENGAGED** (first feature in a long run): named client `"Tugeny"` + `.AddJuggerHubResilience(config,"Tugeny")` + config in appsettings/.env.sample/compose/test factory; breaker **MinimumThroughput 4** derived from ~5 calls per import (breaker counts ATTEMPTS, inside retry); oversize bodies need an **inner `ResponseSizeLimitHandler`** throwing a NON-HTTP exception — `MaxResponseContentBufferSize` throws `HttpRequestException`, which the shared retry would treat as transient. Import is **stateless preview → commit that RE-FETCHES** (provenance is server-established; a finalized Tugeny tournament can't change). Ranking `PUT` carries placement `id`s so an event admin's re-save keeps a platform admin's connection to a non-entrant — without the `id` rule it is either dropped or smuggled past the entrant check. `TeamId` SetNull + `Name`/`SourceName` snapshots (EventParticipation precedent); `ConnectedByUserId` Restrict (award precedent, survives 037 erasure). **Never write `EventParticipation`** (it switches on profile activity + team "active" flags). One recorded deviation: the placement list is a bare list capped at 128 (Roster precedent). **Gate 7 IS engaged** — the binding risk is a **5th admin bottom-bar tab** at 375px in German.)

Prior plan, MERGED: `specs/048-chat-unhide/plan.md` (Hiding a chat is reversible — GH #222. Hiding a conversation is **one-way** today: no un-hide control exists anywhere, and nothing on the send path clears the flag, so a hidden **group, team, party or admin-contact** thread is unreachable through the interface for as long as it is set (a hidden DM has one accidental route: the picker resolves it by person). **THE ISSUE'S 1/2/3 MENU WAS NOT THE REAL QUESTION** — the owner first settled what hide *is*, which 019 never did: FR-026 offered it as the stand-in for *leave* (implying permanence) while the word promises an **archive** (implying temporary), and those cancel out, which is why nothing was built to reverse it. **Owner decision: hide = ARCHIVE, mute = "don't bother me"**, one job each; and FR-026's premise is **rejected outright** — there is no reason to stay on a team but leave its chat, since leaving the team leaves the chat (FR-025), so **mute** becomes the named leave substitute. Build is #222 options **1 + 2**; **option 3 (a "Hidden chats" list/filter) is OUT of scope**, FR-016, follow-up. **NO ENTITY, NO COLUMN, NO MIGRATION, NO ENDPOINT, NO DTO FIELD, NO REALTIME EVENT, NO DEPENDENCY** — if a task produces an `Add-Migration`, something is wrong. **THREE CODE FACTS MAKE THIS TINY, ALL FOUND BY READING**: (1) **the backend already un-hides** — `PatchConversationStateRequest` is nullable-per-flag and `PatchStateAsync` applies `isHidden:false` today; nothing in the product ever *sent* it, so the explicit half is a pure UI gap. (2) **the client already re-seeds the inbox for an unknown conversation** — `ChatService.bumpConversationWithMessage` takes a `!found` branch straight into `loadInbox()` (written for 022's lazy DMs, it describes the un-hide case exactly), reached from **both** the own-send path L292 and the socket handler L473, so **FR-008/FR-010 hold with ZERO new frontend code**; adding an inbox-insert path or a `conversationUnhidden` event is building what already works. (3) **FR-011 is satisfied by NOT writing code** — `WriteSystemMessageAsync` already neither pushes realtime nor bumps `LastMessageDate`, so leaving it alone *is* the implementation; the hazard is that it sits adjacent to `SendAsync` and a future unification would silently take FR-011 with it (guarded by a test + a comment saying why it is not shared). Backend edit is **ONE statement** in `SendAsync`, and **all four of its parts are load-bearing**: placed **between `SaveChangesAsync` and `PushMessageToOthersAsync`** (that method recomputes each recipient's badge and `UnreadTotalAsync` excludes hidden rows — clear it *after* and the conversation returns to the inbox **uncounted by the nav badge**, FR-009); `&& p.IsHidden` keeps it matching **zero rows** in the common case (participant rows are created **lazily** by `EnsureParticipantStateAsync`, so only players who actually hid it carry the flag); `&& p.LeftDate == null` because a group's leaver **keeps** their row — without it, rejoining silently un-hides what they archived; and **`SetProperty(ModifiedDate)` is mandatory** since `ExecuteUpdateAsync` bypasses the change tracker so `AuditFieldsInterceptor` never runs (constitution Principle III — **the single most likely gate failure in the diff**). **Owner clarifications**: only a message **written by a person** returns a conversation — **system lines (joined/left/archived) do NOT** (they'd let roster churn resurface a deliberately archived chat); and a player's **own send returns it to their own inbox too** (today you can post a message invisible in your own inbox). Frontend: `hide()` → `toggleHide()` mirroring `toggleMute()`, with the one asymmetry FR-004 requires — **hide navigates to `/chat`, un-hide stays put**; plus a **real defect fix** — `setState` tests `if (patch.isHidden)` (truthy), so `{isHidden:false}` falls through *both* branches and updates nothing, which is exactly what FR-003 forbids (fix with explicit `=== true` / `=== false` and **independent** `if`s, which also stops a both-flags patch silently dropping the mute). `data-testid` `hide-chat`→`toggle-hide` verified safe (appears only in that template, in no test). One i18n key `chat.details.unhide` × **all three catalogues at once** or `catalog-parity.spec.ts` goes red. **⚠ Principle VII is NOT engaged** — no outbound call and no new browser→backend call; wrapping a local `UPDATE` in retry/breaker is review-rejectable. **Gate 7 IS engaged** (conditional label + new copy) → `checklists/ui-review.md`; the binding string is German **"Wieder in meinen Nachrichten anzeigen"** at 375px. Amend 019 like 022/046 did (callout + FR-026/FR-029 superseded + `contracts/chat-api.md` pointer), but **leave FR-018 alone** — what "hidden" means to the badge is unchanged, only *when* a conversation stops being hidden. Recorded residuals: the clear is **not transactional** with the send (a failure leaves the status quo, self-correcting on the next message — making it atomic would wrap chat's hottest write path in a retriable transaction for a cosmetic flag); one extra `UPDATE` per send; no sweep on release; and whether system lines count toward unread at all is 019 behaviour, deliberately **not** "fixed" here.)

Prior plan, MERGED: `specs/047-chat-message-encryption/plan.md` (Encrypt chat message bodies at rest + require verified TLS on the Postgres hop — GH #223, owner picked options **1 + 2** and **declined option 3 (end-to-end encryption)** outright. **NO NEW ENTITY, NO NEW TABLE, NO NEW ENDPOINT, NO NEW DEPENDENCY** (`AesGcm` is BCL, cert-manager is already installed) — one column, one singleton service, one migration, one connection string, three cert-manager manifests, ONE boolean on `MessageDto`, one i18n key ×3, one corrected privacy paragraph ×3. **⚠ THE ISSUE'S OWN MECHANISM DOES NOT WORK AND WAS REJECTED BY READING THE CODE**: #223 proposes "an EF value converter on `Body` keeps every projection unchanged" — a converter has no access to request culture (so it cannot produce FR-009's localized placeholder), **throws inside `ToListAsync()` materialisation** (one bad row fails the whole conversation — the behaviour the owner rejected), and never sees the row id (so the ciphertext cannot be bound to it). Instead **`ChatMessage.Body` (`string`, `varchar(2000)`) is REPLACED by `ChatMessage.BodyCipher` (`byte[]`, `bytea`)** and decryption happens in the chat services' existing DTO-mapping step, where culture and id are both already in hand. **The rename IS the safeguard** — 5 call sites are forced to acknowledge it (`ChatMessageService` L100/286/340/413/468, `ChatConversationService` L456/499) and a future projection that leaks ciphertext into a DTO is visibly wrong, not invisibly right. **`bytea` makes FR-013 structural**: `ILike` over a byte array does not compile, so the `ILIKE` 046 deleted cannot creep back. **TWO LOAD-BEARING FACTS**: (1) **empty means empty** — system lines and deleted messages store a **zero-length array**, NEVER `Protect("")` (29 bytes of envelope look exactly like a short message and "the content is genuinely gone from the row", 019 R12, would stop being observable); `Protect("")` throws, and `ChatDeleteTests` L72 carries over as `Assert.Empty`. (2) **PostgreSQL's key-file rule is NOT "0600"** — owned by the DB user ⇒ no group/world bits at all; owned by **root** ⇒ group *read* is allowed, which is why a k8s secret at `default_mode="0640"` + `fs_group=70` (the `postgres` uid in `postgres:18.3-alpine`) works, `0644` crash-loops the database with a permissions error that says nothing about TLS, and a Windows bind mount can't be used directly (local copies the key inside the container via a `docker-entrypoint.sh postgres` re-entry wrapper). Envelope = `[version:1][nonce:12][tag:16][ciphertext:n]`, AES-256-GCM, **AAD = the message's UUIDv7 `Id`** (assigned in `BaseEntity`'s field initialiser, so it exists before the insert) — an operator with write access cannot relocate a ciphertext between rows. Keys are **ONE secret**, `Chat__Encryption__Keys="2:<b64>;1:<b64>"`, **first entry writes** — one value means there is nothing to keep in sync, and rotation is "prepend the new key"; startup **fails fast** with no disable switch, mirroring the Redis guard in `Program.cs` L426-443. Connection string verified against pinned Npgsql 10.0.3: the keywords are exactly **`SSL Mode=VerifyFull`** and **`Root Certificate`**; **`Trust Server Certificate` must appear nowhere** and `Host=postgres` must be in the cert SAN (VerifyFull checks the host *as written*). **Owner decisions**: Postgres hop only (Redis TLS/AUTH is *stated* for #219, not built — nothing deploys Redis anywhere yet); `VerifyFull` not `Require`; per-message placeholder not a failed request; several key versions with the first designated current. The placeholder text lives in the **frontend** catalogues (`chat.conversation.messageUnavailable` ×3, guarded by `catalog-parity.spec.ts`), NOT in C# — server-assembled prose has no key to be missing (GH #141). **⚠ Principle VII is NOT engaged** — encryption is local CPU and TLS goes onto a connection the app already makes; `AddJuggerHubResilience`/retry/breaker here is review-rejectable, and a failed cert check is not a transient fault. **Gate 7 IS engaged** (new message state + prose in 6 catalogues) → `checklists/ui-review.md`. Recorded residuals: `ssl=on` **enables** TLS but does not refuse a plaintext client (`pg_hba` left alone — replacing it on Dev's already-initialised PVC is a plausible way to break the database); the Testcontainers Postgres is deliberately **not** given a certificate; the inbox preview does not distinguish unavailable from deleted; Umami gets `?sslmode=require` (encrypted, not verified).)

Prior plan, MERGED: `specs/046-chat-inbox-search/plan.md` (Chat inbox search finds **conversations by name, never message text** — GH #221, amends 019 US6. The owner rejected message-body search outright: typing a name narrows the inbox to the player's own conversations in which **another current member's name matches OR the shown conversation name matches** (group name, team name, an inquiry's team/event name); results ARE inbox rows, in inbox order. **NO ENTITY, NO MIGRATION, NO NEW ENDPOINT, NO NEW DEPENDENCY** — `GET /chat/conversations` gains an optional `q`, and the search is *the inbox query with one more `WHERE`* appended to `VisibleConversations` (the shared `ChatGuard.IsMemberOf` + hide + block exclusions), so same-eligibility / same-row / same-order / unchanged-without-a-term (FR-003/005/006, SC-007) hold by construction; a separate search query would be the third copy of the membership predicate 019 already watched drift once. **THE #1 TRAP IS THE SHARED ENDPOINT**: `/chat/search`'s **people** half is load-bearing for `chat-new`, `chat-compose` (handle→id + existing DM on reload) and `profile-quick-actions` (the Message button) — only the inbox ever consumed its **messages** half. Delete `SearchMessagesAsync`, `MessageSearchHitDto` and `ChatSearchResultDto.Messages` (FR-010: removed, not hidden — a dormant half that reads message bodies is what Principle I exists to shrink) but KEEP the `{ people }` envelope so those three callers stay byte-identical, and never narrow the picker's open reach (019 FR-049). **The name predicate is ONE expression, `ChatGuard.MatchesName`, mirroring `IsMemberOf`'s per-kind branches** (Participants for Direct/Group/Archived; TeamMemberships for Team; PartyMembers `In` for Party; requester + admins for the two inquiry kinds) — Team/Party participant rows are per-user *state*, not membership, so a Participants-only predicate silently misses every teammate who never opened the team chat. Read member names ONLY through `db.PlayerProfiles.Any(...)` (the ban `HasQueryFilter` makes banned/erased members unmatchable for free, and `User.Profile!` navigation misbehaves against it — 044's lesson), **exclude the caller** (their own name is in every conversation), match display name OR handle, `ILike`+`Unaccent`. The fallback labels "Party chat"/"Group"/"Team chat" are NOT matched — recorded as minor drift on FR-002. **Owner decisions (spec Clarifications)**: conversations-filtered, not a people list and not two sections; chat names match too; **hidden stays hidden** (search mirrors the inbox; hide is one-way today — GH #222 filed, OUT of scope). Frontend: `ChatService.searchInbox()` must **never overwrite `_conversations`** (the SignalR-patched signal) — results live in the component's own signal and ONE `@for` renders `displayed = isSearching ? results : conversations`, so clearing restores the inbox instantly and a live message mid-search patches the right list. The inbox loads only its first 20 with **no load-more**, which is why server-side search is the only route to conversation 21+ (SC-004). i18n: reword `chat.inbox.searchSr`/`searchPlaceholder` and delete `inYourMessages`/`people`/`chat` in **all three catalogues at once** — `catalog-parity.spec.ts` goes red otherwise. Tests: 4 message facts in `ChatSearchTests` deleted, 2 rewritten against people, plus an assertion that the response has **no `messages` property**; new `ChatInboxSearchTests`. **⚠ Principle VII is NOT engaged** — no network call, a local SQL predicate; wrapping it in resilience is review-rejectable. **Gate 7 IS engaged** (markup + copy change) → `checklists/ui-review.md`. Amend 019 like 022 did: an "Amended by feature 046" callout, FR-034/035/036/050c + SC-006 marked superseded, a pointer in `contracts/chat-api.md`.)

Prior plan, MERGED: `specs/045-wizard-draft-persistence/plan.md` (Wizard drafts survive leaving the page — GH #182. The create-training and create-event wizards keep every answer in component memory, so in-app navigation, back, a reload, or a **backgrounded mobile tab being evicted** returns the user to a blank step 1. Both wizards get a **`sessionStorage` draft** — written on every answer change, restored before first render, cleared on accepted create / trainings' Cancel / sign-out. **FRONTEND ONLY: no backend, no endpoint, no entity, no migration, no new dependency, and ZERO new i18n keys in the main catalogues.** If a task produces an `Add-Migration` or touches `backend/`, something is wrong. **THE #1 TRAP IS THE CITY CHIP**: `CityPickerComponent` consumes `@Input() initial` in **`ngOnInit`** and `AddressFieldsComponent` carries an explicit ⚠ about it — a value pushed in later never reaches the chip. So the restore MUST happen in the **field initialiser** (not `ngOnInit`, not async), and **`[initialCity]` must be ADDED to both create templates** — today only the three *edit* forms pass it, because only they ever had a stored city. Get this wrong and the wizard restores 20 of 21 fields with an **empty city chip** while the review step confidently prints the restored city. **THE TRAINING WIZARD NEEDS ITS PLAIN FIELDS CONVERTED TO SIGNALS** (10 of them) so one `effect()` can persist everything: the app is **zoneless** and an `effect()` cannot see plain properties — the same hazard the file already documents for `locationKind`/`virtualLink`, whose `[ngModel]="x()" (ngModelChange)="x.set($event)"` form at line 92 is the idiom to copy. Saving only inside `next()` is **not sufficient** (FR-005) — it loses the address-and-description step, the expensive one, to exactly the mid-step eviction that was reported. Wrapping the steps in an `NgForm` is a trap: each step lives in an `@if`, so its controls **unregister on leave** and vanish from `form.value`. Event wizard needs no refactor (`form.valueChanges` for 13 controls) but its **7 signals sit OUTSIDE the FormGroup** and must be in the same effect — count 21 at review. **`busy`/`submitting`/`error` are NEVER persisted** (restoring `busy:true` permanently disables the submit button); clear the draft **only after the server accepts**, never on the click, or a rejected create discards the user's work at the worst moment. "Pristine" for the empty-draft rule is **not** all-blank — trainings default `weekday` to *today*, times to 19:00/21:00; events default limit 16 / cap 8 / EUR — so compare against a snapshot, never hand-written emptiness checks. **OWNER DECISIONS, all three against the issue's options**: draft persistence ONLY (no `canDeactivate`, no `beforeunload`, no step-in-route); **every field persisted INCLUDING `feeIban`+`feeRecipientName`**; and **silent restore with NO discard control** — whose accepted residual is that an abandoned event draft, IBAN included, returns in the same tab. That IBAN decision is the *entire* reason the medium is **`sessionStorage`, not `localStorage`** (dies with the tab) plus a clear-on-sign-out hook in `AuthService.logout()`/`clearSession()`. **The privacy policy MUST be corrected in all 3 locales** (German authoritative): the `legalBasis` sentence "Ours stores nothing there — no cookie, no local storage, nothing" becomes false; the no-banner conclusion survives on the § 25 TDDDG strictly-necessary limb. **Do NOT touch the `analytics` section's own "stores nothing on your device" claims** — 038 verified the recorder uses no storage API and those stay true. `legal-catalog.spec.ts` DM-1 **already** enforces key parity (verified by reading it) — editing `en.json` alone turns the suite red; **no new guard is needed**. **⚠ Principle VII is NOT engaged** — this feature adds no network call at all; wrapping a `setItem` in retry/breaker is review-rejectable. **Gate 7 is recorded as not instantiated**: the silent restore ships no new markup, component, copy or style, so there is no surface for a UI review checklist.)
