...
- [x] T014 [P] ~~Add `EventMapping` Mapster config~~ **Superseded** — DTOs are built with manual EF `.Select(...)` projections (matching the 005 Teams slice and constitution Principle III), which is the codebase's dominant practice; Mapster is used in only one place project-wide (`AuthService`). Whether to standardize on Mapster or projections across the project is tracked as **GitHub issue #15** (with pros/cons).
