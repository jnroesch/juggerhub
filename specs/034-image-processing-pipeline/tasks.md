...
- [X] T019 Run [quickstart.md](./quickstart.md) validation: `dotnet build backend` (clean under `TreatWarningsAsErrors`) and `dotnet test backend/tests/JuggerHub.Api.IntegrationTests`; confirm all new/updated tests pass. **Verified 2026-07-31**: `dotnet build JuggerHub.slnx` → 0 warnings / 0 errors; `dotnet test JuggerHub.slnx` → 567/567 passing. The first run (deferred to CI) was red and exposed two defects, both fixed: the WebP encoder defaulted to **lossless** (ignoring `Quality`, output larger than the source), and two test fixtures used high-frequency `x ^ y` noise that PNG compresses better than any lossy codec.
