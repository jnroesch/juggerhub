...
- [ ] T018a [US2] **Raised by T017, needs an owner decision**: `url_query` is stored alongside `url_path` — the run recorded `/sign-in` with `returnUrl=%2F`. FR-008 decided *paths* are recorded verbatim, but query strings were not considered, and `returnUrl` carries deep links such as `/players/<handle>`. This widens the personal-data surface beyond what FR-008 weighed. Either accept it explicitly or strip queries; feeds the deferred privacy policy (GH #92) and T037
