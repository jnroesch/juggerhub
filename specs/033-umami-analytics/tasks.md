...
- [ ] T014 [US1] Compose the tracker snippet value exactly as specified in [contracts/tracker-snippet.md](./contracts/tracker-snippet.md) — `async` + `defer`, `data-website-id`, `data-do-not-track="true"`, the DNT/GPC guard, **no** `data-host-url`, **no** `identify()`, **no** `data-tag`
- [ ] T015 [US1] Bootstrap locally: start the analytics profile, sign in to Umami, create a website, copy its ID into `.env` as `UMAMI_WEBSITE_ID`, build `JH_ANALYTICS_HEAD`, restart the frontend ([quickstart.md](./quickstart.md) Scenario 3 preamble)
- [ ] T016 [US1] Verify a full page load is recorded and that `/jh-insights.js` loads and `/jh-insights/e` returns 2xx, both on the app's own origin (DevTools Network)
- [ ] T017 [US1] Verify **SPA navigation** produces one page view per screen without a document reload ([quickstart.md](./quickstart.md) Scenario 3 — FR-001, US1 scenario 2). The tracker is **confirmed to hook `pushState`/`replaceState`**, so this is expected to pass and T018 is unlikely to fire — but confirm end-to-end against the real Angular router rather than assuming the hook is sufficient
- [ ] T018 [US1] **Conditional — only if T017 fails**: implement an explicit `umami.track()` call on the Angular router's `NavigationEnd`. This is the one task that touches Angular source; size it before starting, and keep `.html`/`.css`/`.ts` separate per constitution Principle VI
...
- [ ] T024 [US1] Add the analytics environment variables to the **frontend** container spec in `infra/modules/app/main.tf` so the deployed image renders its template with the deployed values (FR-020)
...
- [ ] T027 [US1] Set the Dev values in `infra/envs/dev.tfvars` (1 replica, `analytics-dev.juggerhub.com`, website ID initially empty so the first apply ships no tracker)
...
- [ ] T033 [US1] Bootstrap the Dev website, record its ID in `infra/envs/dev.tfvars`, re-apply, and confirm Dev page views appear (second phase of the two-phase bootstrap in [contracts/configuration.md](./contracts/configuration.md))
...
- [ ] T037 [US2] Inspect the stored `website_event` and `session` rows and confirm no column holds a full IP address and none links to a platform member ([quickstart.md](./quickstart.md) Scenario 6 — FR-005, SC-005). Confirm rather than assume; page paths appearing verbatim is the one expected exception (FR-008)
...
- [ ] T040 [P] [US3] Audit the chosen path names against the forbidden token list in [contracts/nginx-routes.md](./contracts/nginx-routes.md) (`analytics`, `track`, `stat`, `collect`, `beacon`, `pixel`, `umami`, …) and confirm neither contains one
...
- [ ] T045 [US4] Set the Prod values in `infra/envs/prod.tfvars` — `analytics.juggerhub.com`, 2 replicas, prod website ID initially empty. Only sizing and hostname differ from Dev; the resource set is identical (constitution Principle V)
...
- [ ] T058 Apply to Prod, bootstrap the Prod website ID into `infra/envs/prod.tfvars`, re-apply, and confirm measurement (SC-001, SC-002)
