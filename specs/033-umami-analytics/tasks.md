...
- [ ] T031 [US1] Apply to Dev and verify: pods ready, `kubectl -n juggerhub logs -l app=umami -c db-init` shows provisioning succeeded, `kubectl -n juggerhub get certificate` reports `Ready=True`, and the dashboard is reachable at `analytics-dev.juggerhub.com`. **First apply surfaced a defect local testing structurally could not**: `JH_ANALYTICS_UPSTREAM` used the short service name, which nginx's `resolver` cannot resolve because it queries kube-dns **directly and applies no `/etc/resolv.conf` search domains** — every tracker request 502'd with `umami could not be resolved (3: Host not found)` while the pod, Service and `kubectl exec … nslookup umami` all looked healthy. Fixed to the FQDN. Docker's embedded DNS resolves bare names natively, so **compose parity is not resolver parity**
...
- [X] T055 **PASSED — 863 tests, all green**: backend 551, frontend unit 280 (49 suites), Playwright e2e 32. Uncovered and fixed a **pre-existing** bug unrelated to analytics: `docker-compose.e2e.yml` never reset `redis`'s pinned `container_name` (added later by 019), so e2e died with `Conflict. The container name "/juggerhub-redis" is already in use` before running a single test whenever a dev stack was up. Run the existing suites and confirm the injected snippet broke nothing: `docker compose -f docker-compose.test.yml up --abort-on-container-exit` and `docker compose -f docker-compose.e2e.yml up --abort-on-container-exit` ([quickstart.md](./quickstart.md) Scenario 10)
- [X] T056 Run `terraform fmt -check` and `tflint` against `infra/` per the existing `terraform-ci.yml` gate — **`terraform fmt -recursive -check` clean and `terraform validate` reports "Success! The configuration is valid."** `tflint` is not installed locally; it runs in `terraform-ci.yml` on the PR
