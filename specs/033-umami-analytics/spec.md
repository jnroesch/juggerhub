...
- **FR-008a**: Query strings MUST NOT be recorded — only the path. Verbatim recording in FR-008 covers the path alone and does not extend to the query. This is a separate owner decision, taken after implementation showed `/sign-in` being recorded with its `returnUrl`, which carries deep links such as `/players/<handle>` and would have re-introduced member identifiers through a route that FR-008 never considered. Enforced in the tracker (`data-exclude-search`), which discards the query **before the request is sent**, so it never reaches the server rather than being dropped on arrival.
