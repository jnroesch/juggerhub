...
- [ ] T003 [P] Initialize the Nx workspace in `frontend/` with a standalone Angular app `web` (`frontend/apps/web/`) and a Playwright e2e project `web-e2e` (`frontend/apps/web-e2e/`); commit `frontend/nx.json`, `frontend/package.json`, `frontend/tsconfig.base.json`
- [ ] T004 [P] Configure Tailwind and map DESIGN.md tokens (colors, spacing, rounded, typography) into `frontend/apps/web/tailwind.config.js` and expose them as CSS variables in `frontend/apps/web/src/styles.css`
