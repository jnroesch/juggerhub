...
- Q: Does the Tugeny link disappear once the tournament is over? → A: **No.** It stays, pointing at the full bracket instead of the live view, so anyone can still look at the whole tournament tree and every match (FR-015). Tugeny keeps that page for finished tournaments whether or not they were finalized.
...
- **FR-015**: A linked event MUST show a link to the Tugeny tournament: its live view while the event has not ended, and its full bracket (tournament tree) for good once it has — with or without recorded results, and whether or not the tournament was finalized on Tugeny.
