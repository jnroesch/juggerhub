...
- **FR-002**: Both pages MUST be reachable in **no more than two clicks from any screen** of the app, in both the signed-out and signed-in states, and on both desktop and mobile layouts.
...
- **Q: What are the operator's imprint particulars?** → A: **OPEN — see Open Questions below.** Structure, routing, design treatment, and translations do not depend on it; the Prod deploy does.

## Open Questions

- **Q1 (imprint particulars — blocks the US2 *content*, not its structure)**: The operator's legal identity — name, postal address, electronic contact, legal form, and any further particulars applicable to it — cannot be derived from the codebase and is legally mandated. [NEEDS CLARIFICATION: What are the operator's imprint particulars, and is the data-protection contact the same address?]

  **Note for the answer**: this repository is **public**. The particulars are legally required to be published on the site regardless, so publishing them is not itself the question — but the plan must decide whether they live in a committed content file or in per-environment configuration, because a committed home address is also a permanent public git-history entry that cannot be withdrawn by editing the file later. This is a plan-phase decision, recorded here so it is not discovered at deploy time.
