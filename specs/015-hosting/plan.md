...
| IV | Secure auth & sessions | Unchanged app-side, and **strengthened by HTTPS from day one**: same-origin ingress + `X-Forwarded-Proto=https` let the backend issue `Secure` httpOnly cookies *(**corrected by GH #246**: this never held — nothing read `X-Forwarded-Proto` and the frontend nginx overwrote it, so `Secure` followed the environment name and Dev, running as `Development`, had none. `Secure` is now explicit configuration, `Auth:Cookies:Secure`, defaulting to true)*; HTTP→HTTPS redirect enforced at ingress; `/hubs` upgrades over `wss` end-to-end. |
