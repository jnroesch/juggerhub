/**
 * Curated [Lucide](https://lucide.dev) glyphs used across JuggerHub, inlined as SVG inner
 * markup (feature 024, grown to cover the whole product by GH #300). Line icons only, drawn
 * on a 24×24 viewBox; the wrapper `<svg>` in `icon.component` supplies the size, the 2px
 * stroke and the colour, so every icon in the product is drawn at one weight.
 *
 * ## Adding one
 *
 * Copy Lucide's inner nodes verbatim, keep the name Lucide gives it, and keep the list
 * alphabetical. Never redraw a glyph by hand and never adjust a path to "fit" a screen: the
 * thing this map exists to prevent is 99 hand-inlined SVGs at five stroke widths, which is
 * exactly what the audit behind #300 found. Do not use a text glyph as an icon (DESIGN.md).
 *
 * The 13 names DESIGN.md calls out as the product's common set are all here, including
 * `compass`, `swords` and `sparkles` — they are part of the declared vocabulary, so a screen
 * that wants one finds it rather than drawing its own.
 */
export const ICONS = {
  ...
  'calendar-plus':
    '<path d="M8 2v4"/><path d="M16 2v4"/><rect width="18" height="18" x="3" y="4" rx="2"/><path d="M3 10h18"/><path d="M16 19h6"/><path d="M19 16v6"/>',
  ...
  'user-plus':
    '<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><path d="M16 11h6"/><path d="M19 8v6"/><circle cx="9" cy="7" r="4"/>',
  users:
