...
public sealed class AccountEnforcementTests
{
    ...
    public async Task Suspended_login_gets_a_distinct_message_banned_gets_generic_failure()
    {
        ...
        // Suspension blocks ONLY login — the public profile stays fully visible.
        var publicProfile = await _factory.CreateClient().GetAsync($"/api/v1/profiles/{handle}");
    ...
    public async Task Ban_hides_the_player_everywhere_and_unban_restores_intact()
    {
        ...
        // Baseline: visible on profile, browse, and roster.
        Assert.Equal(HttpStatusCode.OK, (await anon.GetAsync($"/api/v1/profiles/{handle}")).StatusCode);
        Assert.Contains(handle, await BrowseAsync(anon, handle));
        ...
        Assert.DoesNotContain(handle, await BrowseAsync(anon, handle));
