...
public sealed class JuggerHubApiFactory : WebApplicationFactory<Program>, IAsyncLifetime
{
    ...
    /// <summary>
    /// A throwaway VAPID pair (feature 055) so the application's fail-fast configuration guard is
    /// satisfied in tests. It is never used to sign a real delivery: <c>IPushDispatcher</c> is
    /// replaced with a recording fake. Generated by <c>scripts/New-VapidKeyPair.ps1</c>.
    /// </summary>
    public const string TestVapidPublicKey =
        "BEJgGLEQANpFt4O8eBgLeWEoDti2bhAECl5wzwHAH-B0d5DVzzclit9DUTzuwa1UnYvQihnUWwVkMe3_QXFHHn4";

    /// <inheritdoc cref="TestVapidPublicKey"/>
    public const string TestVapidPrivateKey = "xB0lyAMcLbC4QYl9sB0w2HnBwYTWXS5VgCYz8iIcCsk";
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureAppConfiguration((_, config) =>
        {
            config.AddInMemoryCollection(new Dictionary<string, string?>
            {
                ...
                // Feature 055 — Web Push. Startup refuses to run without VAPID configuration (there
                // is deliberately no disable switch), so the keys must exist even for tests that
                // never push. A throwaway pair generated by scripts/New-VapidKeyPair.ps1; it
                // signs nothing that leaves the test host, because IPushDispatcher is faked below.
                ["WebPush:Subject"] = "mailto:tests@juggerhub.invalid",
                ["WebPush:PublicKey"] = TestVapidPublicKey,
                ["WebPush:PrivateKey"] = TestVapidPrivateKey,
                // Tight limits and a near-zero backoff so a resilience test finishes quickly. The
                // host in a subscription endpoint is unroutable by design, following the
                // Tugeny:BaseUrl = "http://tugeny.invalid/" precedent: a test that forgets to fake
                // the dispatcher fails fast instead of reaching the real internet.
                ["Resilience:Outbound:WebPush:AttemptTimeoutSeconds"] = "2",
                ["Resilience:Outbound:WebPush:TotalTimeoutSeconds"] = "6",
                ["Resilience:Outbound:WebPush:BaseDelaySeconds"] = "0.01",
